...
3. If the surface has a loading state (skeleton component, Suspense/loading fallback, or query-loading branch), confirm it mirrors the shipped layout — same grid/stack, element count and order, container width, and major spacing — so content replacing the skeleton causes no layout shift.
4. If the validation cannot be performed (sandbox without browser, CI environment), **say so explicitly** in the report rather than implying validation happened.
...
- **Skeleton ↔ rendered-content parity.** Every loading state (skeleton component, Suspense/loading fallback, or query-loading branch) must mirror the structure of the content it stands in for — same layout (grid columns vs. stack), same element count and order, same container width and major spacing — so the swap from skeleton to loaded content produces no layout shift (CLS) or jarring reflow. When you change a surface's rendered structure, update its skeleton in the **same change**: a skeleton that still mirrors the old layout is a regression, not a deferral. The skeleton is a design deliverable, not boilerplate — name it in the plan's Visual States (`loading` entry) and treat "skeleton matches the shipped layout" as part of the acceptance gate above.
