...
- **All CTAs and content links MUST resolve.** Distinct from the design-migration rule above: any URL that ships into rendered output — an internal route link, a server-rendered anchor, a CTA button mapped to a route, a form action, a fetch-then-redirect, an email-template href, or any link embedded in seeded/fixture content — must resolve to a real destination on the first click. This applies to:
  1. **Internal routes**: the link target must match an existing route in the application. The plan must name the concrete route file for each link, not the *intended* route shape. Routes that don't exist yet either get built in the same plan or the link gets removed; never both ship.
  2. **Outbound URLs in seeded content**: every external URL added to seeds or content fixtures must be a verified live URL. Plans that touch seeds or fixtures must include a verification step in the Test Matrix: probe each new URL pre-merge, and pin the URL shape in a test so a future regression breaks CI rather than ships silently.
  3. **Email / notification templates**: every href must use the absolute, base-URL-prefixed form AND target a route that exists. A regression here ships straight to recipients' inboxes — provider delivery logs are not a substitute for a unit test that asserts each href shape.
  4. **Links that depend on data**: a link whose target is constructed from a data field is only safe if the underlying record will exist. Plans that ship new links of this shape must enumerate the seed / pipeline path that guarantees the record.

  The expected output in the File Plan: every CTA listed alongside (a) its target URL, (b) the route file or external URL it resolves to, and (c) — for outbound URLs in seeds — a confirmation that the URL returns a 200 on the page matching the cited content. Reviewers MUST reject plans that ship a link without proving the target resolves.
